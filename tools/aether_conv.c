...
/* Burst of `count` rapid broadcasts (back-to-back, no pacing) to stress the DECT
 * channel: each TX makes the next one's LBT busier, so pre-0.7.29 several would
 * fail with -EBUSY. With the retry-on-busy fix the modem absorbs it -> all OK. */
static int do_bsendn(int count, const char *msg)
{
	uint8_t rb[256];
	char buf[300];
	if (do_version() || do_attach(0)) { fprintf(stderr, "setup failed\n"); return 1; }
	...
	for (int i = 0; i < count; i++) {
		int l = snprintf(buf, sizeof(buf), "%s #%d", msg, i);
		if (do_write(2, buf, l) < 0) { fail++; fprintf(stderr, "  burst %d/%d: FAILED (busy surfaced!)\n", i + 1, count); }
		else ok++;
	}
	printf("[bsendn] conv %d: %d/%d rapid broadcasts sent OK, %d failed\n", conv, ok, count, fail);
	do_clunk(2); do_clunk(1); do_clunk(0);
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	int bsendn = arg2 && strcmp(arg2, "--bsendn") == 0;
	...
	if (bsendn) {
		if (argc < 5) { fprintf(stderr, "usage: %s <sock> --bsendn <count> <msg>\n", argv[0]); return 2; }
		return do_bsendn(atoi(argv[3]), argv[4]);
	}
	if (crecv) {
