...
**Goal.** Let a cyberdeck (a dumb 9p4z client over L2CAP) mount a *remote* mesh
node's filesystem — `dev`/`net` of a node 2 hops away — without the deck needing
any mesh-routing code. The deck does `mount l2cap!dect-modem` and then sees remote
nodes appear as ordinary directories it can `bind` into its namespace. All the
aether routing happens **inside the relay (5340)**.

This is the on-device home for the one piece the 9P-over-mesh demo ran on the host:
`tools/aether_conv --bridge`. Everything else already exists.
