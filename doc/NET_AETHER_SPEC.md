...
### `<N>/data`
- **write** — one datagram to the bound peer. Reliable. Payloads larger than the
  PHY MTU are fragmented by the server (§6); the writer just writes its message.
...
| `announce` | Bind to receive reliable datagrams from **any** peer (server side). `data` reads are source-prefixed (§4). | MUST |
...
3. `data`: one whole datagram per read/write; block reads until datagram/EOF;
   source-prefix announced reads; reliable, in-order, de-duplicated delivery.
